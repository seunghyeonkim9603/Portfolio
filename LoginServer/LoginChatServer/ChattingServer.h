...
public:
	...
	unsigned int	GetTotalLoginSuccessCount() const;
	unsigned int	GetTotalInvalidSessionKeyCount() const;
...
private:
	...
	static unsigned int __stdcall redisThread(void* param);
...
private:
	...
	HANDLE					mhRedisCompletionPort;

	cpp_redis::client	mRedis;
	std::vector<HANDLE> mhRedisThreads;
	...
	unsigned int mNumLoginSuccess;
	unsigned int mNumSessionKeyNotMatched;
