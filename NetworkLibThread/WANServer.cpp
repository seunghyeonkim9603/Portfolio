...
namespace NetworkLib
{
		...
		, const unsigned char fixedKey, INetworkServerEventListener* listener)
	{
		...
		for (unsigned int i = 0; i < mNumContent; ++i)
		{
			mContents[i]->Run();
		}
	...
	void WANServer::RegisterContentThread(ContentThread* content)
	{
		mContents[mNumContent] = content;
		++mNumContent;
	}
	...
	unsigned int __stdcall WANServer::acceptThread(void* param)
	{
		...
		ContentThread* startThread = server->mContents[0];
		...
		while (true)
		{
			...
			Session* session = &server->mSessions[index];
			{
				...
				session->CurrentThreadId = startThread->mThreadId;
			...
			startThread->mJoinedSessions.Enqueue(session);
		...
		while (true)
		{
			...
			else
			{
				...
				if (overlapped == nullptr)
				{
					...
					&& errorCode != WSAECONNRESET && errorCode != ERROR_OPERATION_ABORTED && errorCode != WSAEINVAL && errorCode != 0)
				{
				...
				case EIOType::Recv:
				{
					...
					while (!recvBuffer.IsEmpty())
					{
						...
						Message* message = Message::Create();
						{
							...
							session->RecvQueue.Enqueue(message);
						}
					}
	...
	void WANServer::onLeaveSessionFromContentThread(Session* session)
	{
	...
	void WANServer::moveSessionTo(unsigned int toThreadId, Session* moved)
	{
		unsigned int numContent = mNumContent;

		for (unsigned int i = 0; i < numContent; ++i)
		{
			if (mContents[i]->mThreadId == toThreadId)
			{
				mContents[i]->mJoinedSessions.Enqueue(moved);
				return;
			}
		}
	}
	...
	void WANServer::releaseSession(Session* session)
	{
		...
		while (session->SendQueue.Dequeue(&message))
		{
			Message::Release(message);
		}
		while (session->RecvQueue.Dequeue(&message))
		{
			Message::Release(message);
		}
		for (DWORD i = 0; i < session->NumSent; ++i)
